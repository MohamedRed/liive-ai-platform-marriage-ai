...
export type Animate1LogoProps = AnimateLogoProps & {
  animate: boolean;
  size?: number;
};

export function AnimateLogo1({animate, size, logo, sx, ...other}: Animate1LogoProps) {
  const ratio = 1.875
  const animationSize = size || 120
  return (
    <>
      {
        animate
          ...
          >
            ...
            >
              {logo ?? <Logo disableLink width={animationSize / ratio} height={animationSize / ratio}/>}
            </Box>
...
export function AnimateLogo2({animate, logo, sx, ...other}: AnimateLogoProps) {
  return (
    <Box
      alignItems="center"
      justifyContent="center"
