...
// Define 52 questions (each with a basis for generating an answer)
const questionsList = [
...
function generateAnswer(question) {
  if (question.includes("family environment")) {
    ...
    return `${Math.floor(Math.random() * 10) + 1}`; // Random rating 1-10
    ...
    return question.includes("flexible")
      ? `${Math.floor(Math.random() * 10) + 1}`
      : "I see Mahr as a symbol of respect and security.";
  } if (question.includes("primary responsibilities")) {
    return "I believe a husband should provide and protect, while a wife offers support and nurture, adjusting to modern roles.";
  } if (question.includes("adapt traditional gender roles")) {
    return `${Math.floor(Math.random() * 10) + 1}`;
  } if (question.includes("household decisions")) {
    return "Decisions should be made jointly after thorough discussion.";
  } if (question.includes("job offer in another city")) {
    return "I would balance career aspirations with family stability through open communication.";
  } if (question.includes("relocating for work")) {
    return "Yes, my views are consistent with a balanced approach to change.";
  } if (question.includes("handle stress") || question.includes("emotional challenges")) {
    return "I typically reflect and sometimes consult with trusted mentors.";
  } if (question.includes("emotional expressiveness")) {
    return `${Math.floor(Math.random() * 10) + 1}`;
  } if (question.includes("psychological support")) {
    return "I value honest conversation and mutual care.";
  } if (question.includes("private marital issue")) {
    return "I would encourage a private discussion to resolve it.";
  } if (question.includes("prefer privacy")) {
    return "I appreciate my privacy and strive to balance it with relationship needs.";
  } if (question.includes("boundaries")) {
    return "I believe clear boundaries are essential for both individual space and a healthy relationship.";
    ...
    return "Yes, my boundaries reflect my core values.";
  } else if (question.includes("ideal financial arrangement")) {
    return "I prefer a balanced approach combining both joint and separate responsibilities.";
  } else if (question.includes("financial transparency")) {
    return `${Math.floor(Math.random() * 10) + 1}`;
  } else if (question.includes("both spouses work")) {
    return "I believe in clear, open discussions about shared expenses and roles.";
  } else if (question.includes("large purchase without consulting")) {
    return "I would address it directly to emphasize mutual decision-making.";
  } else if (question.includes("unauthorized purchases")) {
    return "I expect our financial decisions to be transparent and mutually agreed upon.";
  } else if (question.includes("want children")) {
    return "I do want children and envision a nurturing, supportive family.";
  } else if (question.includes("raise children with strong Islamic values")) {
    return `${Math.floor(Math.random() * 10) + 1}`;
  } else if (question.includes("type of schooling")) {
    return "I lean towards Islamic schooling to uphold our cultural and religious values.";
  } else if (question.includes("move abroad for better opportunities")) {
    return "I would discuss the pros and cons extensively with my spouse before deciding.";
  } else if (question.includes("aspirations for children consistent")) {
    return "Yes, my aspirations for children match my openness to new experiences.";
  } else if (question.includes("polygamy")) {
    return question.includes("transparency")
      ? `${Math.floor(Math.random() * 10) + 1}`
      : "I consider polygamy under strict ethical conditions, but I prefer monogamy.";
  } else if (question.includes("views on divorce")) {
    return "I view divorce as a last resort after all reconciliation attempts have been exhausted.";
  } else if (question.includes("proposes divorce")) {
    return "I would pursue counseling and mediation before making a final decision.";
  } else if (question.includes("stance on polygamy align")) {
    return "Yes, my views on polygamy align with my overall approach to marital challenges.";
  } else if (question.includes("mental health challenges")) {
    return "I believe in compassionate support, including professional help if needed.";
  } else if (question.includes("supports you through mental or emotional struggles")) {
    return `${Math.floor(Math.random() * 10) + 1}`;
  } else if (question.includes("personal goals")) {
    return "I hope for mutual support in spiritual, career, and personal growth.";
  } else if (question.includes("signs of depression or spiritual apathy")) {
    return "I would offer support and encourage seeking professional guidance.";
  } else if (question.includes("mutual support match the kind of help")) {
    return "Yes, I believe in providing tangible, practical support during challenging times.";
    ...
    return "I believe open communication can reconcile any conflicting desires.";
  } else {
    return "Sample answer that reflects my view.";
  }
}

// Generate 52 question objects with answers and timestamps
...
function generateProfile(i) {
  ...
  return {
    ...
    dob: {
      ...
      year: 1980 + Math.floor(Math.random() * 20)
    },
...
// Generate 12 primary profiles and for each, create a corresponding 99% match profile.
for (let i = 1; i <= 12; i++) {
  const primaryProfile = generateProfile(i);
  profiles.push(primaryProfile);

  // Create a 99% match profile: deep copy and adjust
  const matchProfile = JSON.parse(JSON.stringify(primaryProfile));
  matchProfile.id = `${String(i).padStart(4, '0')}_for_${primaryProfile.id}`;

  // Adjust one answer slightly to simulate a 99% match
...
// Write the resulting array (24 profile objects total) to profiles.json
fs.writeFileSync("profiles.json", JSON.stringify(profiles, null, 2));
console.log("Generated profiles.json with", profiles.length, "profiles.");
